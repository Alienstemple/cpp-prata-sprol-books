// erase.cpp -- удаление записи из связного списка
...
void eraseRecord(studentCollection sc, int stuNum);
double averageRecord(studentCollection sc); 
void showList (studentCollection sc);
int main()
{
...
void eraseRecord(studentCollection sc, int stuNum)
{
    ...
    while (newNode != NULL)
    {
        if (newNode->studentNum == stuNum)
        {
            newNode = newNode->next;                  // перескок через эл-т и присваивание полей следующего
            while (newNode->next != NULL)
            {
                sc->studentNum = newNode->studentNum;
                sc->grade = newNode->grade;
        ...
        newNode = newNode->next;                // одновременный инкремент нового списка и исходного
        sc = sc->next;
    }
...
void showList (studentCollection sc)
{
    std::cout << std::endl << "Our list:\n";
    listNode * loop = sc;
    int count = 0;
    while (loop != NULL)
    {
        std::cout << "Student " << count + 1 << " number: " << loop->studentNum << std::endl;
        std::cout << "Student " << count + 1 << " grade: " << loop->grade << std::endl;
        loop = loop->next;
        count++;
    }
}